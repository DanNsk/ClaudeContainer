<#
.SYNOPSIS
    Changes the status of an Azure DevOps pull request thread.

.DESCRIPTION
    Updates the status of a comment thread (resolve, close, reopen, etc.).
...
.EXAMPLE
    .\Set-AzDoPullRequestThreadStatus.ps1 -PullRequestUrl "https://dev.azure.com/contoso/WebApp/_git/backend/pullrequest/123" -ThreadId 7 -Status "fixed" -AccessToken $token

.EXAMPLE
    .\Set-AzDoPullRequestThreadStatus.ps1 -Organization "contoso" -Project "WebApp" -RepositoryId "backend" -PullRequestId 123 -ThreadId 7 -Status "fixed"
#>
param(
...
$url = "https://dev.azure.com/$Organization/$Project/_apis/git/repositories/$RepositoryId/pullRequests/$PullRequestId/threads/$ThreadId`?api-version=7.1"
...
try {
    $null = Invoke-RestMethod -Uri $url -Headers $headers -Method Patch -Body $jsonBody
    Write-Output "OK"
} catch {
    Write-Error "Failed to update thread status: $_"
    exit 1
}
